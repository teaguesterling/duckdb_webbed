...
KNOWN BLIND SPOT IN THE GAPS ARM -- it fails in the hiding direction.
...
the exporter by hand -- but a value short enough to appear in unrelated source is
outside what it can detect, and no allowlist entry should be written as though it
were load-bearing without checking that it fires.
...
INTENTIONAL_GAPS = {
    # INERT TODAY: never fires -- see the GAPS blind spot above.
