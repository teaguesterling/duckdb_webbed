...
CREATE OR REPLACE MACRO _resolve(p) AS CASE WHEN p IS NULL THEN NULL WHEN p[1] = '/' THEN p ELSE '/mnt/aux-data/teague/Projects/duckdb_webbed/trees/issue/38-date-format/' || p END;
CREATE OR REPLACE MACRO _session_root() AS '/mnt/aux-data/teague/Projects/duckdb_webbed/trees/issue/38-date-format';
...
.output
SELECT mcp_server_start(COALESCE(getvariable('transport'), 'stdio'), getvariable('mcp_server_options')) WHERE COALESCE(getvariable('transport'), 'stdio') <> 'none';
