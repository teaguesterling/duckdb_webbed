...
void XMLSchemaInference::CollectChildElements(xmlNodePtr parent, const std::string &child_name,
                                              const XMLSchemaOptions &options, std::vector<xmlNodePtr> &result) {
	if (!parent) {
		return;
	}
...
std::vector<XMLColumnInfo> XMLSchemaInference::InferSchema(const std::string &xml_content,
	...
	// max_depth=2: children are records (depth 1), columns at depth 2, remaining = 2-2 = 0 (can see columns but nested
	// become XML) max_depth=3: children are records (depth 1), columns at depth 2, remaining = 3-2 = 1 (can introspect
	// 1 level deep)
...
std::vector<xmlNodePtr> XMLSchemaInference::IdentifyRecordElements(XMLDocRAII &doc, xmlNodePtr root,
	...
	if (!options.record_element.empty()) {
		...
		if (xpath_expr.find('/') == std::string::npos && xpath_expr.find('[') == std::string::npos &&
			...
			if (tag_part.find('/') == std::string::npos && tag_part.find('[') == std::string::npos &&
...
std::vector<ElementPattern> XMLSchemaInference::AnalyzeDocumentStructure(const std::string &xml_content,
	...
	if (!record_attributes.empty()) {
		std::string record_element_name =
		    record_elements.empty() ? "record" : std::string((const char *)record_elements[0]->name);
...
std::vector<Value> XMLSchemaInference::ExtractSingleRecord(xmlNodePtr record,
                                                            const std::vector<XMLColumnInfo> &schema,
...
std::vector<Value> XMLSchemaInference::ExtractSingleRecordWithSchema(xmlNodePtr record,
		...
		if (attr_value) {
			...
			if (child == nullptr) {
				...
				if (record_name == column_name) {
					value = ExtractValueFromNode(record, column_type);
				} else {
...
std::vector<std::vector<Value>> XMLSchemaInference::ExtractDataWithSchema(const std::string &xml_content,
	...
	// Extract data from each record element using single-record function
	for (xmlNodePtr record : record_elements) {
		rows.push_back(ExtractSingleRecordWithSchema(record, column_names, column_types, options));
	}
